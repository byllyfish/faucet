...
def _apply_configs_existing(dp_id, new_dp):
    APP.logger.info('Reconfiguring existing datapath %s', dpid_log(dp_id))
    valve = APP.valves[dp_id]
    cold_start, flowmods = valve.reload_config(new_dp)
    if flowmods:
        if cold_start:
            APP.metrics.faucet_config_reload_cold.labels( # pylint: disable=no-member
                **valve.base_prom_labels).inc()
            APP.logger.info('Cold starting %s', dpid_log(dp_id))
        else:
            APP.metrics.faucet_config_reload_warm.labels( # pylint: disable=no-member
                **valve.base_prom_labels).inc()
            APP.logger.info('Warm starting %s', dpid_log(dp_id))
        _send_flow_msgs(new_dp.dp_id, flowmods)
        return valve
    APP.logger.info('No changes to datapath %s', dpid_log(dp_id))
    return None

def _apply_configs_new(dp_id, new_dp):
    APP.logger.info('Add new datapath %s', dpid_log(dp_id))
    valve_cl = valve_factory(new_dp)
    if valve_cl is not None:
        return valve_cl(new_dp, APP.logname, APP.notifier)
    APP.logger.error(
...
def _apply_configs(new_dps):
    ...
    for new_dp in new_dps:
        ...
        if dp_id in APP.valves:
            valve = _apply_configs_existing(dp_id, new_dp)
        else:
            valve = _apply_configs_new(dp_id, new_dp)
    ...
    for deleted_valve_dpid in deleted_valve_dpids:
        ...
        ryu_dp = zof.find_datapath(datapath_id=deleted_valve_dpid)
...
async def start(_):
    ...
    APP.stat_reload = get_setting('FAUCET_CONFIG_STAT_RELOAD')
    ...
    APP.notifier = zof_event_notifier.FaucetExperimentalEventNotifier(
        get_setting('FAUCET_EVENT_SOCK'), APP.metrics, APP.logger)
    ...
    zof.ensure_future(_periodic_task(_lldp_beacon, 5))
...
def channel_up(event):
    ...
    # FIXME(bfish): In ryu land, features_reply is called *before* the 
    # dpset.EventDP/connect_or_disconnect handler is called. In the future, 
    # the Channel_Up event will have a copy of the FeaturesReply msg. For now, 
    # pass None for the FeaturesReply message (it's not used).
    flowmods = APP.valves[dp_id].switch_features(None)
...
def flow_removed(event):
    ...
    if msg['reason'] == 'IDLE_TIMEOUT':
        match = pktview_from_list(msg['match'])
        flowmods = valve.flow_timeout(msg['table_id'], match)
        _send_flow_msgs(dp_id, flowmods)
...
def _valve_flow_services(valve_service):
...
def _resolve_gateways():
    ...
    _valve_flow_services('resolve_gateways')


def _state_expire():
    """Handle a request expire host state in the controller."""
    _valve_flow_services('state_expire')
...
def _advertise():
    ...
    _valve_flow_services('advertise')
...
def _lldp_beacon():
    ...
    _valve_flow_services('send_lldp_beacons')
