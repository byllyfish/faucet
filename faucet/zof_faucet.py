...
def channel_up(event):
    ...
    flowmods = APP.valves[dp_id].datapath_connect(up_port_nums)
...
def channel_down(event):
    ...
    APP.valves[dp_id].datapath_disconnect()
...
def packet_in(event):
    ...
    APP.metrics.faucet_packet_in_secs.labels(dp_id=hex(dp_id)).observe(packet_in_stop - packet_in_start)
    _send_flow_msgs(dp_id, flowmods)
